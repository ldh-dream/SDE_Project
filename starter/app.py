...
def search_venues():
  ...
  for result in search_result:
    data.append({
      "id":result.id,
      "name": result.name,
      "num_upcoming_shows":db.session.query(func.count(Show.c.Venue_id)).filter(Show.c.Venue_id == result.id).filter(Show.c.start_time > datetime.datetime.now()).all()[0][0]
    })
  response = {
...
def show_venue(venue_id):
  ...
  upcoming_shows_query= db.session.query(Artist, Show).filter(Show.c.Venue_id==venue_id).filter(Artist.id == Show.c.Artist_id).filter(Show.c.start_time>datetime.datetime.now()).all()
  upcoming_shows = []
  ...
  past_shows_query = db.session.query(Artist, Show).filter(Show.c.Venue_id == venue_id).filter(Artist.id == Show.c.Artist_id).filter(Show.c.start_time < datetime.datetime.now()).all()
  past_shows = []
...
def search_artists():
  ...
  for result in search_result:
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": db.session.query(func.count(Show.c.Artist_id)).filter(Show.c.Artist_id == result.id).filter(
        Show.c.start_time > datetime.datetime.now()).all()[0][0]
    })
  response = {
    "count": len(search_result),
    "data": data
...
def show_artist(artist_id):
  ...
  upcoming_shows_query = db.session.query(Venue, Show).filter(Show.c.Venue_id == Venue.id).filter(
    artist_id == Show.c.Artist_id).filter(Show.c.start_time > datetime.datetime.now()).all()
  upcoming_shows = []
  for show in upcoming_shows_query:
    upcoming_shows.append({
  ...
  past_shows_query = db.session.query(Venue, Show).filter(Show.c.Venue_id == Venue.id).filter(
    artist_id == Show.c.Artist_id).filter(Show.c.start_time < datetime.datetime.now()).all()
  past_shows = []
  for show in past_shows_query:
    past_shows.append({
...
def shows():
  ...
  shows = db.session.query(
    ...
    Show).filter(Show.c.Venue_id == Venue.id).filter(Show.c.Artist_id == Artist.id).all()
  data = []
  for show in shows:
    data.append({
      "venue_id": show.venue_id,
